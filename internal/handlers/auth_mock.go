...
// Source: /home/sergey/Github/bil-message/internal/handlers/auth.go
...
// MockDeviceAdder is a mock of DeviceAdder interface.
type MockDeviceAdder struct {
	ctrl     *gomock.Controller
	recorder *MockDeviceAdderMockRecorder
}

// MockDeviceAdderMockRecorder is the mock recorder for MockDeviceAdder.
type MockDeviceAdderMockRecorder struct {
	mock *MockDeviceAdder
}

// NewMockDeviceAdder creates a new mock instance.
func NewMockDeviceAdder(ctrl *gomock.Controller) *MockDeviceAdder {
	mock := &MockDeviceAdder{ctrl: ctrl}
	mock.recorder = &MockDeviceAdderMockRecorder{mock}
	return mock
}

// EXPECT returns an object that allows the caller to indicate expected use.
func (m *MockDeviceAdder) EXPECT() *MockDeviceAdderMockRecorder {
	return m.recorder
}

// AddDevice mocks base method.
func (m *MockDeviceAdder) AddDevice(ctx context.Context, username, password, publicKey string) (uuid.UUID, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "AddDevice", ctx, username, password, publicKey)
	ret0, _ := ret[0].(uuid.UUID)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}
...
func (mr *MockDeviceAdderMockRecorder) AddDevice(ctx, username, password, publicKey interface{}) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "AddDevice", reflect.TypeOf((*MockDeviceAdder)(nil).AddDevice), ctx, username, password, publicKey)
}
